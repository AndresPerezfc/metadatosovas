...
<div class=container>
    <div class="list-group">
        <a href="#" class="list-group-item list-group-item-action flex-column align-items-start active">
            ...
            <h5 class="mb-1">{{$ova->titulo}}</h5>
            <small>{{$ova->fecha}}</small>
            </div>
            <p class="mb-1">{{$ova->descripcion}}</small>
        </a>
    </div>
</div>
...
<table class="table table-bordered">
  <thead>
    <tr>
      ...
      <th scope="col">Tipo de interactividad</th>
      <th scope="col">Recurso de aprendizaje</th>
      <th scope="col">Nivel de interactividad</th>
      <th scope="col">Población objetivo</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <th scope="row">{{$ova->id}}</th>
      <td>{{$ova->idiomas->nombre}}</td>
      <td>{{$ova->formatos->nombre}}</td>
      <td>{{$ova->tiposInteractividad->nombre}}</td>
      <td>{{$ova->recursosAprendizaje->nombre}}</td>
      <td>{{$ova->nivelesInteractividad->nombre}}</td>
      <td>{{$ova->poblacionesObjetivo->nombre}}</td>
    </tr>
  </tbody>
</table>
</div>
